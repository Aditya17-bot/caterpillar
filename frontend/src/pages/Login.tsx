...
export default function Login() {
  ...
  const [selected, setSelected] = useState<Doc<'operators'> | null>(null)

  const [faceStage, setFaceStage] = useState<FaceStage>('idle')
  const [faceOperator, setFaceOperator] = useState<Doc<'operators'> | null>(null)
  ...
  function scanCard(operator: Doc<'operators'> | null) {
  ...
  function startFaceVerification(operator: Doc<'operators'>) {
  ...
  function enterPlatform(doc: Doc<'operators'>) {
    ...
    <div className="min-h-screen w-full bg-surface flex items-center justify-center p-space-lg relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-3xl flex flex-col gap-space-lg">
        ...
        <span className="text-center font-label-sm text-label-sm text-outline uppercase tracking-widest">
          Simulated authentication for hackathon demonstration — no real RFID or biometric hardware connected.
        </span>
      </div>
    </div>
  )
